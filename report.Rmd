...
branch.adjSibling.cor <- cor.test(~ branch.adjSibling$Document..updateStyle + I(branch.adjSibling$branchiness ^ 2))
branch.genSibling.cor <- cor.test(~ branch.genSibling$Document..updateStyle + I(branch.genSibling$branchiness ^ 2))
```
...
The data confirms this idea. The time to match general sibling selectors fits a linear regression model on the square of the DOM branching factor (p < `r branch.genSibling.cor$p.value`).

```{r, echo=FALSE}
ggplot(branch.genSibling, aes(x = branchiness, y = Document..updateStyle)) +
...
Surprisingly, adjacent sibling selectors follow a quadratic trend as well (p < `r branch.adjSibling.cor$p.value`).

```{r, echo=FALSE}
ggplot(branch.adjSibling, aes(x = branchiness, y = Document..updateStyle)) +
